...
export default async function ReportsPage() {
  ...
  const statusStats = {
    normal: monthlyRecords?.filter(r => r.status === 'normal').length || 0,
    late: monthlyRecords?.filter(r => r.status === 'late').length || 0,
    absent: monthlyRecords?.filter(r => r.status === 'absent').length || 0,
    overtime: monthlyRecords?.filter(r => r.status === 'overtime').length || 0,
    leave: monthlyRecords?.filter(r => r.status === 'leave').length || 0,
  }

  const pieData = [
    { name: '正常', value: statusStats.normal, color: '#22c55e' },
    { name: '迟到', value: statusStats.late, color: '#eab308' },
    { name: '旷工', value: statusStats.absent, color: '#ef4444' },
    { name: '加班', value: statusStats.overtime, color: '#3b82f6' },
    { name: '请假', value: statusStats.leave, color: '#a855f7' },
  ].filter(d => d.value > 0)
